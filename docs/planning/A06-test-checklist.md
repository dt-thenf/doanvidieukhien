...
- [ ] EVT_ORDER_NEW: trên Pi chạy tool với `--send-evt-order-new` → PIC beep + OLED bếp hiện “EVT_ORDER_NEW”.
- [ ] EVT_PAYMENT_PENDING: trên Pi chạy tool với `--send-evt-payment-pending` → PIC beep + OLED quầy hiện “EVT_PAYMENT_PENDING”.
...
- [ ] Keypad 4×4 scan cơ bản: nhấn `1 2 3` thấy chuỗi tích luỹ trên OLED quầy; `#` clear; `*` backspace.
- [ ] OLED init + clear + print text: khi reset thấy “PIC KITCHEN/COUNTER BOOT”; nhấn MODE đổi màn hình hiển thị “READY”.
- [ ] NRF TX-first: nhấn `K_DONE` / `A` / `B` tạo frame 32 byte và `nrf_bridge_send()` trả true khi TX_DS (poll STATUS).
- [ ] NRF RX cơ bản: khi Pi gửi `EVT_ORDER_NEW` / `EVT_PAYMENT_PENDING`, PIC nhận frame 32 byte và cập nhật OLED/buzzer tương ứng.
