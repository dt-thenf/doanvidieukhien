...
3. **Một PIC cho hai cụm IO:** bếp (buzzer, **OLED**, nút) và quầy (keypad 4×4, **OLED**) — firmware phải có **chế độ** rõ ràng.
...
| Hệ quả | Tài liệu demo cần một dòng hướng dẫn “đang ở chế độ nào” trên OLED. |
...
| Hệ quả | OLED quầy hiển thị đúng đơn vị tiền Việt Nam trong phạm vi demo; API snapshot đồng bộ cùng quy ước. |
...
|--------|----------|
| Trạng thái | **Đã chốt** |
| Bối cảnh | NRF mất gói; PIC không bắt buộc ACK `EVT_*` (**D-2026-04-12-02**); vẫn cần hành vi Pi đơn giản để demo ổn định. |
...
## D-2026-04-14-01 — Dev-only `kitchen-done` khi `PI_DEBUG=1` (Agent A04.1)

| Trường | Nội dung |
|--------|----------|
| Trạng thái | **Đã chốt (MVP hỗ trợ test)** |
| Bối cảnh | Sau foundation, beginner khó test chuỗi “gửi đơn → DONE → yêu cầu thanh toán” vì `CMD_KITCHEN_DONE` chưa có NRF/HTTP. |
| Quyết định | Khi **`PI_DEBUG=1`**, mount **`POST /api/v1/dev/tables/{table_id}/kitchen-done`** gọi cùng `apply_kitchen_done` như PIC; khi debug tắt, route **không mount** (404). **Không** bật debug trên bản demo công khai nếu không chủ đích. |
| Hệ quả | Test local và Swagger rõ ràng; `api-contract.md` / README ghi rõ dev-only; không đổi kiến trúc Pi/PIC/NRF. |

---

## D-2026-04-13-03 — Lớp ingress PIC (`pic_ingress`) tái sử dụng `apply_*` (Agent A05.2)

| Trường | Nội dung |
|--------|----------|
| Trạng thái | **Đã chốt (MVP triển khai)** |
| Bối cảnh | Cần chỗ gom tín hiệu PIC/NRF (hoặc mock) vào Pi mà **không** nhân đôi nghiệp vụ với HTTP dev; chuẩn bị firmware PIC16F887. |
| Quyết định | Thêm package `app/services/pic_ingress/`: `PicIngressIn(command, table_code)` + `handle_pic_ingress` → chỉ gọi `apply_kitchen_done` / `apply_counter_lookup` / `apply_counter_paid`. Adapter mock `decode_pic_command_json` (UTF-8 JSON). Route `dev` gọi **cùng** `handle_pic_ingress`. |
| Hệ quả | Worker RF sau này chỉ cần parse binary → `PicIngressIn`; tài liệu `docs/architecture/pic-ingress.md` + test `test_pic_ingress.py`. |

---

## D-2026-04-13-04 — Binary protocol `v1` + worker NRF + skeleton PIC (Agent A06)

| Trường | Nội dung |
|--------|----------|
| Trạng thái | **Đã chốt (MVP triển khai)** |
| Bối cảnh | Cần frame 32 byte cố định (NRF), `table_code` (u16 LE) thống nhất với `PicIngressIn`, và worker Pi không nhân đôi `apply_*`. |
| Quyết định | **SoT:** `docs/architecture/pi-pic-protocol.md` — bảng `MSG_TYPE` hex, payload `CMD_*` như mục 4. **Pi:** `nrf_binary.decode_pic_command_binary`, helpers `build_cmd_*_frame`, `worker.handle_nrf_ingress_frame` / `run_ingress_loop`. **Firmware:** `firmware/pic16f887/include/pi_pic_proto.h` + `src/main.c` (skeleton). Không thêm route HTTP dev mới. |
| Hệ quả | Test `tests/test_nrf_binary.py`; checklist `docs/planning/A06-test-checklist.md`. |

---

## D-2026-04-13-05 — Repo hygiene & source-of-truth (Agent A05.2)

| Trường | Nội dung |
|--------|----------|
| Trạng thái | **Đã chốt** |
| Bối cảnh | Hai cây `docs/` (root vs `restaurant-pi-pic/docs/`), artifact Stitch `v2-*.png` dễ nhầm với bản A02.3, và từ “mock” dễ gây hiểu nhầm với ingress/API thật. |
| Quyết định | Thêm `docs/architecture/source-of-truth.md`: **SoT** = `docs/` tại root; `restaurant-pi-pic/docs/` = mirror/bundle planning; UI Stitch chốt = `.stitch/designs/` (`01`–`04`, HTML + PNG); ảnh `v2-*.png` chuyển `.stitch/designs/archive/`. Cập nhật `README.md` root, `docs/architecture/README.md`, `docs/decisions/README.md`, `restaurant-pi-pic/docs/architecture/README.md`. |
| Hệ quả | Agent sau (A06/firmware) tra cứu một map; không đổi kiến trúc hay nghiệp vụ. |

---

## D-2026-04-13-06 — Pin reassignment NRF `CSN` (A06.1a)

| Trường | Nội dung |
|--------|----------|
| Trạng thái | **Đã chốt (bring-up firmware)** |
| Bối cảnh | Pin map ban đầu dùng `RA3` cho NRF `CSN`, nhưng firmware giữ `MCLRE = ON` và quy ước không dùng `RA3` làm output để tránh xung đột với MCLR trong một số mạch/thiết kế beginner. |
| Quyết định | Đổi NRF `CSN` từ **`RA3` → `RB5`**. Giữ `CE = RA4`, `IRQ = RB0/INT`, SPI `RC3/RC4/RC5`. |
| Hệ quả | Cập nhật `docs/architecture/pic-pin-map.md` + `firmware/pic16f887/include/pin_map.h` và init I/O. Khi đi dây phần cứng, nối `CSN` vào `RB5`. |

---

## D-2026-04-13-07 — PORTB output safety (shadow) (A06.1b)

| Trường | Nội dung |
|--------|----------|
| Trạng thái | **Đã chốt (bring-up firmware)** |
| Bối cảnh | `RB0` (NRF IRQ, input) + `RB1..RB4` (keypad rows, output) + `RB5` (NRF CSN, output) cùng trên PORTB. Ghi rải rác `PORTBbits.RBx` dễ gây khó debug/đụng bit khác khi mở rộng keypad/NRF thật. |
| Quyết định | Thêm module `portb_safe.*` dùng **shadow byte** cho các output do firmware “sở hữu” (`RB1..RB5`). Mọi thao tác `CSN` và keypad rows ưu tiên đi qua helper (`portb_set_nrf_csn`, `portb_set_keypad_rows`, ...). |
| Hệ quả | Giảm rủi ro thao tác bit lẻ; dễ audit khi debug. Không đổi pin map hay kiến trúc tổng thể. |

---

## D-2026-04-13-08 — Chiến lược NRF TX-first (A06.3)

| Trường | Nội dung |
|--------|----------|
| Trạng thái | **Đã chốt (bring-up firmware)** |
| Bối cảnh | Cần giảm rủi ro tích hợp: trước khi làm RX/IRQ/ACK đầy đủ, firmware nên chứng minh được đường SPI + điều khiển CE/CSN + phát gói 32 byte ổn định. |
| Quyết định | Triển khai **TX-first** trong `nrf_bridge.*`: init MSSP SPI, cấu hình NRF tối thiểu (TX mode) và hàm `nrf_bridge_send(frame32)` pulse CE để phát, polling `STATUS` để biết TX_DS/MAX_RT. RX/IRQ để vòng sau. |
| Hệ quả | Cho phép test phần cứng theo thao tác nút/keypad, tạo nền cho bước nối ACK/RX sau này mà không đổi kiến trúc/state machine. |

---

## D-2026-04-13-09 — NRF RX polling + ACK matching (A06.4)

| Trường | Nội dung |
|--------|----------|
| Trạng thái | **Đã chốt (bring-up firmware)** |
| Bối cảnh | Sau TX-first, cần nhận được `EVT_*` và `ACK/NACK/PONG` từ Pi để hoàn thiện vòng transaction tối thiểu mà vẫn giữ firmware đơn giản, dễ debug. |
| Quyết định | Implement **RX polling** trong `nrf_bridge_tick()` (đọc `STATUS.RX_DR` → `R_RX_PAYLOAD` 32 byte) và expose qua `nrf_bridge_try_recv()`. `ACK/NACK` chỉ kết thúc `WAIT_ACK` khi **`SEQ` khớp** frame vừa gửi. |
| Hệ quả | Không cần IRQ interrupt phức tạp ở vòng này; đủ để nối với Pi backend và quan sát OLED/buzzer phản ứng theo event. |

---

## D-2026-04-25-01 — Đổi hiển thị PIC từ LCD sang 2 OLED SSD1306 (A06.2b)

| Trường | Nội dung |
|--------|----------|
| Trạng thái | **Đã chốt** |
| Bối cảnh | Pin RC3/RC4/RC5 đang phục vụ NRF SPI; phần LCD HD44780 (4-bit) tiêu tốn nhiều pin và không còn phù hợp với phần cứng demo hiện tại. Cần vẫn giữ 2 màn hình độc lập cho **bếp** và **quầy** nhưng **không đổi kiến trúc tổng thể** (Pi trung tâm + 1 PIC + NRF). |
| Quyết định | Thay LCD bằng **2 màn OLED 0.96 inch SSD1306 128×64**. OLED dùng **software I2C (bit-bang)** trên pin **`RD0=SCL`, `RD1=SDA`**. Hai OLED dùng chung bus I2C, địa chỉ 7-bit: **kitchen=`0x3C`**, **counter=`0x3D`**. |
| Hệ quả | Firmware chuyển `lcd_driver.*` → `oled_driver.*` (text-mode: init/clear/print ngắn + chọn màn theo address). Phần cứng cần pull-up cho I2C. Nếu thực tế 2 module OLED có cùng địa chỉ (thường `0x3C`), cần đổi jumper/address hoặc dùng I2C mux; dự án vẫn giữ `0x3C/0x3D` làm SoT. |

---

## D-2026-04-13-02 — Dev HTTP `counter-paid` khi `PI_DEBUG=1` (Agent A05.1)

| Trường | Nội dung |
|--------|----------|
| Trạng thái | **Đã chốt (MVP hỗ trợ test)** |
| Bối cảnh | Beginner test full flow local cần bước **quầy chốt tiền** (`CMD_COUNTER_PAID`) trước khi có firmware PIC / NRF; trước đó chỉ có dev `kitchen-done`. |
| Quyết định | Khi **`PI_DEBUG=1`**, thêm **`POST /api/v1/dev/tables/{table_id}/counter-paid`** gọi cùng `apply_counter_paid` như PIC; khi debug tắt → **404**. **Không** coi HTTP này là chốt tiền production; bản thật vẫn do PIC. Optional: frontend chỉ hiện nút gọi route này khi `VITE_ENABLE_E2E_DEV_PANEL=1`. |
| Hệ quả | E2E local: đơn → kitchen-done → payment request → counter-paid → admin reset bàn; tài liệu `api-contract.md`, `pi-backend-flow.md`, README pi-backend đồng bộ. |

---

## D-2026-04-13-01 — Scaffold pi-backend MVP local (Agent A04)

| Trường | Nội dung |
|--------|----------|
| Trạng thái | **Đã chốt (triển khai MVP)** |
| Bối cảnh | Cần backend chạy local khớp stack FastAPI + SQLModel + SQLite và API tối thiểu cho 2 web; chưa NRF thật. |
| Quyết định | Thêm thư mục `pi-backend/` với SQLite `create_all`, seed demo, REST `/api/v1/customer/*` và `/api/v1/admin/*`, stub `PicBridge` (EVT_ORDER_NEW / EVT_PAYMENT_PENDING + retry 1 lần) và hàm xử lý `CMD_*` trong `pic_commands.py` (chưa HTTP). MVP **không** dùng `PENDING_AT_COUNTER`. |
| Hệ quả | OpenAPI tại `/docs`; tài liệu `docs/architecture/db-schema.md`, `api-contract.md`, `pi-backend-flow.md` mô tả schema và luồng; frontend có thể nối API ở vòng sau. |
...
*Cập nhật gần nhất: agent **A06** (D-2026-04-13-04 — binary `v1` + worker + skeleton PIC); **A05.2** (D-2026-04-13-05 — repo hygiene / SoT; D-2026-04-13-03 — `pic_ingress`); **A05.1** (D-2026-04-13-02); **A04.1** (D-2026-04-14-01); **A04** (D-2026-04-13-01); **A01.6** (D-16–D-19); A01.5 (stack + design workflow); A01 (giao thức `v1`).*
